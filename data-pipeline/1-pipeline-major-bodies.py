"""
NASA Horizons → ExoAtlas JSON/CSV Uploader (Planets + Dwarf Planets)
--------------------------------------------------------------------
...
R2_CSV_KEY  = os.environ.get("R2_CSV_KEY",  "major-bodies.csv")
...
def parse_horizons_elements(response_text: str) -> Optional[Dict[str, Any]]:
    ...
    Parses the Horizons ELEMENTS block:
      ...
      N MA TA
      A AD PR
    Returns dict with a,e,i,Ω,ω,ν, epoch JDTDB string.
    """
    ...
    epoch_jd = lines[0].split("=")[0].strip()  # "<JDTDB>"
    values = {}
...
def r2_upload_csv(csv_path: Path, bucket: str, key: str) -> str:
    s3 = _r2_client()
    extra = {
        "ContentType": "text/csv; charset=utf-8",
        "CacheControl": R2_CACHE_CONTROL,
        # Optional: set ContentDisposition for easy download
        # "ContentDisposition": f'inline; filename="{csv_path.name}"',
    }
...
def main() -> int:
    ...
    # 3) JSON → /tmp (+ R2) and optional local file
    try:
        ...
        # --- Uploads to R2 ---
        have_r2 = all([R2_ENDPOINT, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET])
        if have_r2:
            # JSON
            if R2_JSON_KEY:
                obj_json = r2_upload_json(TMP_JSON_PATH, R2_BUCKET, R2_JSON_KEY)
                print(f"[ok] R2 uploaded JSON: {obj_json}")
            else:
                print("[skip] R2_JSON_KEY not set")

            # CSV
            if R2_CSV_KEY:
                obj_csv = r2_upload_csv(TMP_CSV_PATH, R2_BUCKET, R2_CSV_KEY)
                print(f"[ok] R2 uploaded CSV: {obj_csv}")
            else:
                print("[skip] R2_CSV_KEY not set")
        else:
            print("[skip] R2 credentials not fully set; skipping uploads")

        # Optional: repo-local JSON for dev preview
        try:
